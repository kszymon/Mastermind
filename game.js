...
function getRandomColors(colors, num) {
    let solution = [];
    let colorsCopy = [...colors]; // Kopiujemy tablicę, aby nie modyfikować oryginału

    for (let i = 0; i < num; i++) {
        let randomIndex = Math.floor(Math.random() * colorsCopy.length);
        solution.push(colorsCopy[randomIndex]);
        colorsCopy.splice(randomIndex, 1); // Usuwamy wybrany kolor, aby nie powtarzał się
    }

    return solution;
}

let solution = getRandomColors(colors, 5);
